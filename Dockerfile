...
RUN echo "-Dsbt.coursier.home=/workspace/.coursier" >> /home/linuxbrew/.linuxbrew/etc/sbtopts \
    ...
    && echo alias "mkdir -p /workspace/.sbt/1.0/plugins/ && initMetals='echo "'addSbtPlugin\\("ch.epfl.scala" % "sbt-bloop" % "1.4.8"\\)'" > /workspace/.sbt/1.0/plugins/metals.sbt && sbt -Dbloop.export-jar-classifiers=sources bloopInstall +Test/compile'" >> .bashrc
